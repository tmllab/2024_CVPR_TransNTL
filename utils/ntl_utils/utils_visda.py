...
def get_augment_data():
    ...
    augment_trainset = os.listdir("./data_ntl_aug/augment_visda/")
    augment_labels = np.loadtxt("./data_ntl_aug/augment_visda/labels")

    for i in range(len(augment_trainset) - 1):
        img = cv2.imread("./data_ntl_aug/augment_visda/img_{}.png".format(i))
        # img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        list_img.append(img)
...
def add_watermark(dataset_list, img_size = 112, value=100):
